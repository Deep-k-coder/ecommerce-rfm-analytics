"""
build_v2_10page_report.py
Generates a redesigned, publication-grade 10-page First Progress Work Report (24/09/2026):
1. Redesigned Cover Page: Elegant minimalist executive aesthetic with IAR Banner at top.
2. NO DEPARTMENT NAME included anywhere in the report (as requested).
3. Candidate: Deep Koshiya | Program: B.Sc. in Artificial Intelligence & Data Science (Semester V).
4. University: Institute of Advanced Research (IAR).
5. Exact 10-page layout budget in exported PDF.
"""
...
def build_report():
    ...
    def add_para(text, bold_prefix=None, space_after=3.5, space_before=0, line_spacing=1.1, italic=False, align=WD_ALIGN_PARAGRAPH.JUSTIFY):
        p = doc.add_paragraph()
        p.alignment = align
    ...
    # =========================================================================
    # PAGE 1: NEW REDESIGNED COVER PAGE (NO DEPARTMENT NAME)
    # =========================================================================
    # 1. Top Hero Banner
    ...
    if iar_banner_path.exists():
        ...
        r_ban.add_picture(str(iar_banner_path), width=Inches(5.6))

    # 2. Institutional Title & Academic Program (No Department Name!)
    ...
    r_prog = p_prog.add_run("B.Sc. in Artificial Intelligence & Data Science | Semester V (5th Sem)\nAcademic Session: 2026 – 2027")
    ...
    # Assessment Card (Clean Slate Shaded Table)
    ...
    # Two-Column Scholar & Supervision Table (No Department Name!)
    ...
    add_para(
        "I express my sincere appreciation to my project supervisor and faculty mentors at the Institute of Advanced Research (IAR) for their technical guidance, critical evaluation, and encouragement throughout the system conception, requirement analysis, and architectural design phases of this project."
    )
    add_para(
        "I also thank the university leadership and the Head of the School for facilitating the computational resources and laboratory environments required to execute Project-1 (CE-502 / PRJ-701) in accordance with our academic timeline."
    )
    add_para(
        "This First Progress Work report, presented for formal evaluation on 24th September 2026, documents the requirements engineering, data refinery architecture, and software design using UML (Sections 1.1, 1.2, 1.3, 2.1, and 2.2). I remain committed to refining these models as the system progresses toward database physical schemas and algorithmic deployment for the upcoming Mid-Term evaluation."
    )
    add_para("Deep Koshiya | B.Sc. in AI & Data Science (Semester V) | Date: 24/09/2026", space_after=10, space_before=2, align=WD_ALIGN_PARAGRAPH.RIGHT, italic=True)

    add_h1("ABSTRACT", space_before=6, space_after=4)
    add_para(
        "In modern multi-channel commerce, treating an entire consumer portfolio as a homogenous base leads to severe financial waste, misallocated Customer Acquisition Costs (CAC), and customer attrition blindspots. The 'E-Commerce Customer Lifecycle & RFM Segmentation Dashboard' resolves these challenges through an end-to-end data analytics and unsupervised machine learning framework."
    )
    add_para(
        "The automated system ingests raw transaction event logs, enforces strict schema validation, isolates unauthenticated sessions, segregates reverse logistics (credit notes and cancellations) into an audit ledger, and neutralizes bulk distribution anomalies using Tukey's Interquartile Range (IQR) fences. A vectorized feature engineering engine computes customer-level Recency, Frequency, and Monetary (RFM) metrics alongside Average Order Value (AOV). Features undergo log1p compression and Z-score standardization to mitigate Pareto spend skewness, enabling robust K-Means clustering validated via Elbow and Silhouette metrics. Concurrently, a rule-based quantile engine (pd.qcut) assigns deterministic 1–5 scores, mapping accounts to 8 operational business cohorts ('Champions', 'Loyal Customers', 'At Risk Whales', etc.)."
    )
    add_para(
        "This report documents the system foundation and software design for the 24/09/2026 assessment milestone at Institute of Advanced Research (IAR). It presents complete Level-0, Level-1, and Level-2 Data Flow Diagrams (DFDs) alongside a comprehensive Use Case Model and specification matrix, fulfilling all requirements for the first 10-mark evaluation."
    )
    ...
    add_para("The system resolves four core operational bottlenecks in digital retail analytics:", bold_prefix="• Core Analytical Needs: ")
    ...
    print(f"[SUCCESS] Built new redesigned report at {out_docx_path}")
